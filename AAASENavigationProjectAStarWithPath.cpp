...
void A () {
    ...
    pq.push({0 + h[start_node], start_node, start_node}); // distance to the start node is 0. This is the base case

    while(!pq.empty()) { // run until we reach every node
        int node = pq.top()[1]; // old node
        int dist = pq.top()[0]; // distance to start node with heuristic
        dist -= h[node]; // distance to start node
        ...
        for (pair<int, int> new_data : adj[node]) { // go to the nodes the old node is connected to
            ...
            if (!visited[child]) {
                pq.push({dist + add_cost + h[child], child, node});
            }
        }
    }
